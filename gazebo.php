...
function gazebo_filter($input) {
    ...
    function gazebo_posts_where($where) {
...
add_action('init','gazebo_addcolumn');
add_action('post_submitbox_misc_actions', 'gazebo_publish_options');
//add_action('attachment_submitbox_misc_actions', array('wp_gazebo_plugin', 'gazebo_publish_options'));
add_action('post_updated', 'gazebo_update_post');
add_filter('the_content', 'gazebo_filter', 12);
add_filter('the_excerpt', 'gazebo_filter');
add_filter('posts_where', 'gazebo_posts_where');
add_action('wp_logout', 'gazebo_logout');
add_filter('wp_authenticate', 'gazebo_login_filter');
add_shortcode( 'gazebo_disp_posts', 'gazebo_disp_posts' );
/* End of File */